...
async function main() {
  ...
  const manifest = {
    ...
    modules: [
      { id: "78cfe1ed-1a6f-4a14-a1c2-92f6d80253b0", name: "Dashboard", icon: "Layout", sortOrder: 0 },
      { id: "fb7aa608-7b9e-4077-a2cb-6fa7c894ddcc", name: "User Management", icon: "Users", sortOrder: 1 },
      { id: "cbe34cea-9716-41c5-a58e-baac2bca2a28", name: "Security & Logs", icon: "History", sortOrder: 2 }
    ],
    pages: [
      { id: "9f3cfde7-f712-4cb5-973c-42519832ed52", name: "Dashboard", path: "/", sortOrder: 0, moduleId: "78cfe1ed-1a6f-4a14-a1c2-92f6d80253b0" },
      { id: "78ae9eb2-a0af-4d61-b7d6-8381ced8a6ab", name: "Users List", path: "/users", sortOrder: 0, moduleId: "fb7aa608-7b9e-4077-a2cb-6fa7c894ddcc" },
      { id: "945087d9-92f8-4b7f-b652-7a97ffa16595", name: "Roles & Permissions", path: "/roles", sortOrder: 1, moduleId: "fb7aa608-7b9e-4077-a2cb-6fa7c894ddcc" },
      { id: "c3eebd62-e64d-4318-9eb8-8ae7b1e82cd6", name: "Audit Logs", path: "/audit-logs", sortOrder: 0, moduleId: "cbe34cea-9716-41c5-a58e-baac2bca2a28" },
      { id: "10f9baa4-7e08-44d5-9b64-dd6993094e2f", name: "System Settings", path: "/settings", sortOrder: 1, moduleId: "cbe34cea-9716-41c5-a58e-baac2bca2a28" },
      { id: "55b0b5f4-8ac3-4505-ad6e-ee59592c5539", name: "Modules Registry", path: "/modules", sortOrder: 2, moduleId: "cbe34cea-9716-41c5-a58e-baac2bca2a28" }
    ],
    rolePermissions: [
      // SUPER_ADMIN
  ...
  for (const page of manifest.pages) {
    const { moduleId, ...pData } = page;
    await prisma.page.upsert({
      where: { id: page.id },
      update: { ...pData, module: { connect: { id: moduleId } } },
      create: { ...pData, module: { connect: { id: moduleId } } }
    });
  }

  // D. Role Permissions
  for (const rp of manifest.rolePermissions) {
    const { roleId, pageId, ...rpData } = rp;
    await prisma.permission.upsert({
      where: { roleId_pageId: { roleId, pageId } },
      update: { ...rpData, role: { connect: { id: roleId } }, page: { connect: { id: pageId } } },
      create: { ...rpData, role: { connect: { id: roleId } }, page: { connect: { id: pageId } } }
    });
  }

  // E. Users
  for (const user of manifest.users) {
    const { roleId, ...uData } = user;
    await prisma.user.upsert({
      where: { id: user.id },
      update: { ...uData, role: { connect: { id: roleId } } },
      create: { ...uData, role: { connect: { id: roleId } } }
    });
  }

  // F. User Overrides
  for (const override of manifest.userPermissions) {
    const { userId, pageId, ...oData } = override;
    await prisma.userPermission.upsert({
      where: { userId_pageId: { userId, pageId } },
      update: { ...oData, user: { connect: { id: userId } }, page: { connect: { id: pageId } } },
      create: { ...oData, user: { connect: { id: userId } }, page: { connect: { id: pageId } } }
    });
  }

  console.log("✅ Golden Seed Successfully Synchronized.");
}
